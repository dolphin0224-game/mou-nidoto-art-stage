...
const keys=['soft','soft','graphite','paint','soft','soft','soft'];
...
function closeDialogue(){command('dialogue-close');$('#dialogue').hidden=true;dialogue=false;canvas.focus()}
...
window.onStageState=s=>{
 window.stageState=s;document.querySelector(".brand small").textContent=["","04 月夜 · 基準","05 鉛筆","＋ 厚塗り","A 繊細な月夜","B 夢と記憶 · 会話でぼけ","C 伝承の挿絵"][s.style];document.querySelectorAll('[data-style]').forEach(b=>b.classList.toggle('selected',Number(b.dataset.style)===s.style));
 ...
 $('#notice').textContent=`小庭園 v9 · X ${s.x.toFixed(1)} / Z ${s.z.toFixed(1)} m · Rで戻る`;
