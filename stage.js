...
const lines={talk:['陽菜','……ここ、静かだね。もう少しだけ、一緒に見ていこう。'],flower:['白い花','夜の青に沈む花びら。その縁だけに、宿の灯りが残っている。'],lamp:['宿の灯り','障子の向こうで灯りが揺れた。風が止んでも、その明るさはわずかに変わり続ける。']};
...
window.onStageState=s=>{
 ...
 $('#motionButton').textContent=s.atmosphere?'風 ON':'風 OFF';$('#motionButton').setAttribute('aria-pressed',s.atmosphere);
 $('#talk').style.left=(s.talkX*100)+'%';$('#talk').style.top=(s.talkY*100)+'%';$('#talk').style.visibility=s.talkVisible?'visible':'hidden';
 $('#notice').textContent=`歩行版 v5 · X ${s.x.toFixed(1)} / Z ${s.z.toFixed(1)} m · Rで戻る`;
